...
def build_pretrain_embedding(embedding_path, word_alphabet, embedd_dim=100, norm=True, load_all=False):
    embedd_dict = dict()
...
def load_pretrain_emb(embedding_path):
    ...
    with open(embedding_path, 'r', encoding="utf8") as file:
        for line in file:
            ...
            #    print  (tokens[0], embedd_dim + 1, len(tokens))
                try:
                    assert (embedd_dim + 1 == len(tokens))
                except:
                    print ("Invalid embedding for token", tokens[0])
                    continue
